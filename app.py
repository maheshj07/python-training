...
def ai_chat():
    ...
    if request.method == 'POST':
        ...
        products_list = [f"{p.name} - Rs.{p.price} - {p.category}" for p in Product.query.all()]
        prompt = f"You are a shop assistant. Products: {products_list}. Answer this: {question}"
...
with app.app_context():
    db.create_all()
    # Sample Products add करणे
    if Product.query.count() == 0:
        sample = [
            Product(name='iPhone 15', category='Mobile', price=79999, stock=10, description='Latest Apple Phone'),
            Product(name='Samsung TV 55"', category='TV', price=55000, stock=5, description='4K Smart TV'),
            Product(name='Sony Headphones', category='Audio', price=15000, stock=20, description='Noise Cancelling')
        ]
